...
class saleClass(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.Payment_type_entry = customtkinter.CTkComboBox(self, width=120, values=["Cash", "Cheque"],
                                                            command=self.refrance)
        ...
        self.no1_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no1var,
                                                        values=list(self.ItemList), command=self.itm1)
        self.no1_item_entry.grid(row=2, column=1, padx=5, pady=5)
        self.no1var.trace('w', self.itemlist_update1)

        self.no2_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no2var,
                                                        values=self.ItemList, command=self.itm2)
        self.no2_item_entry.grid(row=3, column=1, padx=5, pady=5)
        self.no2var.trace('w', self.itemlist_update2)

        self.no3_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no3var,
                                                        values=self.ItemList, command=self.itm3)
        self.no3_item_entry.grid(row=4, column=1, padx=5, pady=5)
        self.no3var.trace('w', self.itemlist_update3)

        self.no4_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no4var,
                                                        values=self.ItemList, command=self.itm4)
        self.no4_item_entry.grid(row=5, column=1, padx=5, pady=5)
        self.no4var.trace('w', self.itemlist_update4)

        self.no5_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no5var,
                                                        values=self.ItemList, command=self.itm5)
        self.no5_item_entry.grid(row=6, column=1, padx=5, pady=5)
        self.no5var.trace('w', self.itemlist_update5)

        self.no6_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no6var,
                                                        values=self.ItemList, command=self.itm6)
        self.no6_item_entry.grid(row=7, column=1, padx=5, pady=5)
        self.no6var.trace('w', self.itemlist_update6)

        self.no7_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no7var,
                                                        values=self.ItemList, command=self.itm7)
        self.no7_item_entry.grid(row=8, column=1, padx=5, pady=5)
        self.no7var.trace('w', self.itemlist_update7)

        self.no8_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no8var,
                                                        values=self.ItemList, command=self.itm8)
        self.no8_item_entry.grid(row=9, column=1, padx=5, pady=5)
        self.no8var.trace('w', self.itemlist_update8)

        self.no9_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no9var,
                                                        values=self.ItemList, command=self.itm9)
        self.no9_item_entry.grid(row=10, column=1, padx=5, pady=5)
        self.no9var.trace('w', self.itemlist_update9)

        self.no10_item_entry = customtkinter.CTkComboBox(self.navigation_frame, width=300, variable=self.no10var,
                                                         values=self.ItemList, command=self.itm10)
        ...
        self.no1_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno1var,
                                                        values=self.Item_unit_List1, command=self.itemshow)
        self.no1_unit_entry.grid(row=2, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update1)

        self.no2_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno2var,
                                                        values=self.Item_unit_List2, command=self.itemshow)
        self.no2_unit_entry.grid(row=3, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update2)

        self.no3_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno3var,
                                                        values=self.Item_unit_List3, command=self.itemshow)
        self.no3_unit_entry.grid(row=4, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update3)

        self.no4_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno4var,
                                                        values=self.Item_unit_List4, command=self.itemshow)
        self.no4_unit_entry.grid(row=5, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update4)

        self.no5_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno5var,
                                                        values=self.Item_unit_List5, command=self.itemshow)
        self.no5_unit_entry.grid(row=6, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update5)

        self.no6_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno6var,
                                                        values=self.Item_unit_List6, command=self.itemshow)
        self.no6_unit_entry.grid(row=7, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update6)

        self.no7_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno7var,
                                                        values=self.Item_unit_List7, command=self.itemshow)
        self.no7_unit_entry.grid(row=8, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update7)

        self.no8_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno8var,
                                                        values=self.Item_unit_List8, command=self.itemshow)
        self.no8_unit_entry.grid(row=9, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update8)

        self.no9_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno9var,
                                                        values=self.Item_unit_List9, command=self.itemshow)
        self.no9_unit_entry.grid(row=10, column=3, padx=5, pady=5)
        self.iuno1var.trace('w', self.itemunit_update9)

        self.no10_unit_entry = customtkinter.CTkComboBox(self.navigation_frame, width=120, variable=self.iuno10var,
                                                         values=self.Item_unit_List10, command=self.itemshow)
        ...
        self.no1_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp1,
                                                            state="readonly")
        self.no1_P_unitprice_entry.grid(row=2, column=4, padx=5, pady=5)

        self.no2_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp2,
                                                            state="readonly")
        self.no2_P_unitprice_entry.grid(row=3, column=4, padx=5, pady=5)

        self.no3_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp3,
                                                            state="readonly")
        self.no3_P_unitprice_entry.grid(row=4, column=4, padx=5, pady=5)

        self.no4_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp4,
                                                            state="readonly")
        self.no4_P_unitprice_entry.grid(row=5, column=4, padx=5, pady=5)

        self.no5_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp5,
                                                            state="readonly")
        self.no5_P_unitprice_entry.grid(row=6, column=4, padx=5, pady=5)

        self.no6_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp6,
                                                            state="readonly")
        self.no6_P_unitprice_entry.grid(row=7, column=4, padx=5, pady=5)

        self.no7_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp7,
                                                            state="readonly")
        self.no7_P_unitprice_entry.grid(row=8, column=4, padx=5, pady=5)

        self.no8_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp8,
                                                            state="readonly")
        self.no8_P_unitprice_entry.grid(row=9, column=4, padx=5, pady=5)

        self.no9_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp9,
                                                            state="readonly")
        self.no9_P_unitprice_entry.grid(row=10, column=4, padx=5, pady=5)

        self.no10_P_unitprice_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ipp10,
                                                             state="readonly")
        ...
        self.no1_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida1,
                                                           state="readonly")
        self.no1_dec_amount_entry.grid(row=2, column=7, padx=5, pady=5)

        self.no2_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida2,
                                                           state="readonly")
        self.no2_dec_amount_entry.grid(row=3, column=7, padx=5, pady=5)

        self.no3_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida3,
                                                           state="readonly")
        self.no3_dec_amount_entry.grid(row=4, column=7, padx=5, pady=5)

        self.no4_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida4,
                                                           state="readonly")
        self.no4_dec_amount_entry.grid(row=5, column=7, padx=5, pady=5)

        self.no5_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida5,
                                                           state="readonly")
        self.no5_dec_amount_entry.grid(row=6, column=7, padx=5, pady=5)

        self.no6_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida6,
                                                           state="readonly")
        self.no6_dec_amount_entry.grid(row=7, column=7, padx=5, pady=5)

        self.no7_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida7,
                                                           state="readonly")
        self.no7_dec_amount_entry.grid(row=8, column=7, padx=5, pady=5)

        self.no8_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida8,
                                                           state="readonly")
        self.no8_dec_amount_entry.grid(row=9, column=7, padx=5, pady=5)

        self.no9_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida9,
                                                           state="readonly")
        self.no9_dec_amount_entry.grid(row=10, column=7, padx=5, pady=5)

        self.no10_dec_amount_entry = customtkinter.CTkEntry(self.navigation_frame, width=120, textvariable=self.ida10,
                                                            state="readonly")
    ...
    def add_invoice_event(self):
        ...
        try:
            if self.partyname_entry.get() == "":
                ...
                cur.execute(f"Update invosale set partyname=?,phonenumber=?,gstin=?,invoiceno=?,invoicedate=?,steteofsuply=?,paymentype=?,refreceno=?,total=?,received=?,balance=?,totaltac=?,totaldec=?,totalqty=?,item1name=?,qty1=?,unit1=?,unitprice1=?,dec1=?,desamount1=?,amount1=?,item2name=?,qty2=?,unit2=?,unitprice2=?,dec2=?,desamount2=?,amount2=?,item3name=?,qty3=?,unit3=?,unitprice3=?,dec3=?,desamount3=?,amount3=?,item4name=?,qty4=?,unit4=?,unitprice4=?,dec4=?,desamount4=?,amount4=?,item5name=?,qty5=?,unit5=?,unitprice5=?,dec5=?,desamount5=?,amount5=?,item6name=?,qty6=?,unit6=?,unitprice6=?,dec6=?,desamount6=?,amount6=?,item7name=?,qty7=?,unit7=?,unitprice7=?,dec7=?,desamount7=?,amount7=?,item8name=?,qty8=?,unit8=?,unitprice8=?,dec8=?,desamount8=?,amount8=?,item9name=?,qty9=?,unit9=?,unitprice9=?,dec9=?,desamount9=?,amount9=?,item10name=?,qty10=?,unit10=?,unitprice10=?,dec10=?,desamount10=?,amount10=? where sid=1",
                    (
    ...
    def get_final_qty(self, entery, input, output,iname):
      try:
        if entery.get() == "" or entery.get() == " " :
            output.set("")
        else:
          name=(iname.get())
          inqty = int(str(entery.get()))
          dbqty = int(str(input.get()))
          if dbqty < inqty:
            ffin =int(inqty - dbqty)
            output.set(ffin)
            self.updateqty(name,ffin,1)
          elif dbqty > inqty:
            ffin =int(dbqty - inqty)
            output.set(ffin)
            self.updateqty(name, ffin, 2)
          elif dbqty == inqty:
            output.set(dbqty)
            self.updateqty(name, dbqty, 3)
      except Exception as e:
         print(e)
    def updateqty(self,name,qtyy,no):
      item_name = name
      if item_name == "" or item_name == " ":
       ...
       try:
        ...
        # row = cur.fetchone()
        cur.execute(f"Update itemdata set openqty=? where pid={pidd}", (
          resualt,
        ))
        con.commit()
       except Exception as e:
    ...
    def qqty(self):
      try:
        self.get_final_qty(self.no1_qty_entry, self.pastqty1, self.fpastqty1,self.no1_item_entry)
        self.get_final_qty(self.no2_qty_entry, self.pastqty2, self.fpastqty2,self.no2_item_entry)
        self.get_final_qty(self.no3_qty_entry, self.pastqty3, self.fpastqty3,self.no3_item_entry)
        self.get_final_qty(self.no4_qty_entry, self.pastqty4, self.fpastqty4,self.no4_item_entry)
        self.get_final_qty(self.no5_qty_entry, self.pastqty5, self.fpastqty5,self.no5_item_entry)
        self.get_final_qty(self.no6_qty_entry, self.pastqty6, self.fpastqty6,self.no6_item_entry)
        self.get_final_qty(self.no7_qty_entry, self.pastqty7, self.fpastqty7,self.no7_item_entry)
        self.get_final_qty(self.no8_qty_entry, self.pastqty8, self.fpastqty8,self.no8_item_entry)
        self.get_final_qty(self.no9_qty_entry, self.pastqty9, self.fpastqty9,self.no9_item_entry)
        self.get_final_qty(self.no10_qty_entry, self.pastqty10, self.fpastqty10,self.no10_item_entry)
      except Exception as e:
         print(e)
    # def getqqty(self):
    #     self.qqty()
    ...
    def get_party_gstin(self):
        ...
        if self.partyname_entry.get() == "" or self.partyname_entry.get() == " ":
            self.gstin.set("")
        else:
         con = sqlite3.connect(database=r'DataBase/ims.db')
         cur = con.cursor()
                    ...
                    for i in row:
                        if i == "":
                            if str(stock) >= "":
                                ...
                                iseto.set("0")
                                messagebox.showerror("Aleart",
                                                     f"Product is in stock {stock} and minum sell qty is {i}!",
                                                     parent=self)
                        else:
                            if stock == 0:
                                iseto.set(str(i))
    ...
    def finalamount(self):
        try:
            ...
            if self.roundoff_check.get() == 1:
                ...
                finalamount = float(sitam1) + float(sitam2) + float(sitam3) + float(sitam4) + float(sitam5) + float(
                    sitam6) + float(sitam7) + float(sitam8) + float(sitam9) + float(sitam10)
    ...
    def itm10(self, event):
        ...
        # self.get_item_tax(self.no10_item_entry, self.itax10)
        self.get_item_dec(self.no10_item_entry.get(), self.id10)
        self.itemgstbill(self.no10_qty_entry.get(), self.no10_unitprice_entry.get(),
                         self.no10_dec_percentagee_entry.get(),
                         self.ida10, self.iam10)
        self.finalamount()
        self.totalqty()
    ...
    def get_data(self):
        ...
        try:
            ...
            self.invo.set(items_datalist[3])
            ...
            self.no2var.set(items_datalist[19])
            self.iq2.set(items_datalist[20])
            self.pastqty2.set(items_datalist[20])
            self.Item_unit_List.insert(0, items_datalist[21])
            self.no2_unit_entry.set(items_datalist[21])
            self.no2_unit_entry.configure(values=self.Item_unit_List)
            self.ip2.set(items_datalist[22])
            self.id2.set(items_datalist[23])

            self.no3var.set(items_datalist[26])
            self.iq3.set(items_datalist[27])
            self.pastqty3.set(items_datalist[27])
            self.Item_unit_List.insert(0, items_datalist[28])
            self.no3_unit_entry.set(items_datalist[28])
            self.no3_unit_entry.configure(values=self.Item_unit_List)
            self.ip3.set(items_datalist[29])
            self.id3.set(items_datalist[30])

            self.no4var.set(items_datalist[33])
            self.iq4.set(items_datalist[34])
            self.pastqty4.set(items_datalist[34])
            self.Item_unit_List.insert(0, items_datalist[35])
            self.no4_unit_entry.set(items_datalist[35])
            self.no4_unit_entry.configure(values=self.Item_unit_List)
            self.ip4.set(items_datalist[36])
            self.id4.set(items_datalist[37])

            self.no5var.set(items_datalist[40])
            self.iq5.set(items_datalist[41])
            self.pastqty5.set(items_datalist[41])
            self.Item_unit_List.insert(0, items_datalist[42])
            self.no5_unit_entry.set(items_datalist[42])
            self.no5_unit_entry.configure(values=self.Item_unit_List)
            self.ip5.set(items_datalist[43])
            self.id5.set(items_datalist[44])

            self.no6var.set(items_datalist[47])
            self.iq6.set(items_datalist[48])
            self.pastqty6.set(items_datalist[48])
            self.Item_unit_List.insert(0, items_datalist[49])
            self.no6_unit_entry.set(items_datalist[49])
            self.no6_unit_entry.configure(values=self.Item_unit_List)
            self.ip6.set(items_datalist[50])
            self.id6.set(items_datalist[51])

            self.no7var.set(items_datalist[54])
            self.iq7.set(items_datalist[55])
            self.pastqty7.set(items_datalist[55])
            self.Item_unit_List.insert(0, items_datalist[56])
            self.no7_unit_entry.set(items_datalist[56])
            self.no7_unit_entry.configure(values=self.Item_unit_List)
            self.ip7.set(items_datalist[57])
            self.id7.set(items_datalist[58])

            self.no8var.set(items_datalist[61])
            self.iq8.set(items_datalist[62])
            self.pastqty8.set(items_datalist[62])
            self.Item_unit_List.insert(0, items_datalist[63])
            self.no8_unit_entry.set(items_datalist[63])
            self.no8_unit_entry.configure(values=self.Item_unit_List)
            self.ip8.set(items_datalist[64])
            self.id8.set(items_datalist[65])

            self.no9var.set(items_datalist[68])
            self.iq9.set(items_datalist[69])
            self.pastqty9.set(items_datalist[69])
            self.Item_unit_List.insert(0, items_datalist[70])
            self.no9_unit_entry.set(items_datalist[70])
            self.no9_unit_entry.configure(values=self.Item_unit_List)
            self.ip9.set(items_datalist[71])
            self.id9.set(items_datalist[72])

            self.no10var.set(items_datalist[75])
            self.iq10.set(items_datalist[76])
            self.pastqty10.set(items_datalist[76])
            self.Item_unit_List.insert(0, items_datalist[77])
            self.no10_unit_entry.set(items_datalist[77])
            self.no10_unit_entry.configure(values=self.Item_unit_List)
            self.ip10.set(items_datalist[78])
            self.id10.set(items_datalist[79])
