...
class saleClass(customtkinter.CTk):
    ...
    def get_data(self):
        ...
        try:
            ...
            cur.execute(
                "select pid,partyname,gstin,phonenumber,gsttype,state,emailid,billaddress,shipaddress,paybalence,recivebalence,date,creditlim,add1,add2,add3,add4 from editgstsale where pid=?",
                (1,))
            rows = cur.fetchall()
            for row in rows:
                for r in row:
